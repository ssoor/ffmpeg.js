...
* `ffmpeg.js` - MP4 && WebM encoding (H.264 & AAC & VP8 & Opus encoders, popular decoders).
* `ffmpeg-web.js` - Web Worker version of `ffmpeg.js`.
...
1.  Clone ffmpeg.js repository with submodules:
    ```bash
    git clone https://github.com/ssoor/ffmpeg.js.git --recurse-submodules
    ```
...
cd ~
git clone https://github.com/ssoor/ffmpeg.js.git --recurse-submodules && cd ffmpeg.js
make
```

## Credits

Thanks to [ffmpeg.js](https://github.com/Kagami/ffmpeg.js/) for inspiration. And of course to all great projects which made this library possible: FFmpeg, Emscripten, asm.js, node.js and many others.
